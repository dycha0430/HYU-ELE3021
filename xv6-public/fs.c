...
bmap(struct inode *ip, uint bn)
{
  ...
  if (bn < NDINDIRECT) {
    ...
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    int next_block_index = bn / NINDIRECT;
    bn -= NINDIRECT * next_block_index;
    if ((addr = a[next_block_index]) == 0) {
      a[next_block_index] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
  ...
  if (bn < NTINDIRECT) {
    ...
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    int next_block_index = bn / NDINDIRECT;
    bn -= NDINDIRECT * next_block_index;
    if ((addr = a[next_block_index]) == 0) {
      a[next_block_index] = addr = balloc(ip->dev);
      log_write(bp);
    }
    ...
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    int triple_block_index = bn / NINDIRECT;
    bn -= NINDIRECT * triple_block_index;
    if ((addr = a[triple_block_index]) == 0) {
      a[triple_block_index] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
